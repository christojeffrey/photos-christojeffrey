  ...
  return (
    <StoreContext.Provider value={[storeData, setStoreData] as any}>
      <div className="relative overflow-hidden">
        {props.children}
        <div className={`fixed top-0 bottom-0 right-0 left-0 ${storeData.isShown ? "visible" : "invisible"}`}>
          {storeData.isShown && (
            <>
              {/* background */}
              ...
              <Image
                src={storeData.photoData.full.url}
                className="p-4"
                alt="image"
                onClick={(e) => {
                  closePreview();
                ...
                placeholder="blur"
                priority
                blurDataURL={storeData.blurData.base64}
              />
            </>
          )}
        </div>
